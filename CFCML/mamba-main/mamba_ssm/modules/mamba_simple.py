...
class Mamba(nn.Module):
    ...
    bimamba_type:
        ...
        v8: cross-scan mamba with single input (shared weights)
        v9: cross-scan mamba with two inputs (shared weights | x, z | B, C, t)
        v10: cross-scan mamba with n inputs (x, z | B, C, t)
    """
    def __init__(
        self,
        d_model,
        ...
        if bimamba_type in ["v0", "v3", "v5", "v7", "v9", "v10"]:
            self.in_proj = nn.Linear(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
            self.in_proj_b = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)
        elif bimamba_type == "v4":
            self.in_proj = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)
        ...
        # bidirectional
        if bimamba_type in ["v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10"]:
            A_b = repeat(
                torch.arange(1, self.d_state + 1, dtype=torch.float32, device=device),
                "n -> d n",
                d=self.d_inner,
        ...
        if bimamba_type in ["v2", "v3", "v4", "v6", "v7", "v10"]:
            self.conv1d_b = nn.Conv1d(
                in_channels=self.d_inner,
                out_channels=self.d_inner,
                bias=conv_bias,
        ...
        if bimamba_type in ["v6", "v7", "v8", "v9", "v10"]:
            A_c = repeat(
                torch.arange(1, self.d_state + 1, dtype=torch.float32, device=device),
                "n -> d n",
                d=self.d_inner,
        ...
        if bimamba_type in ["v10"]:
            self.att = nn.Sequential(nn.Conv1d(input_num*token_len, token_len, kernel_size=5, stride=1, padding=2),
                                            nn.BatchNorm1d(token_len),
                                            nn.ReLU(),
                                            nn.Conv1d(token_len, token_len, kernel_size=1, stride=1, padding=0),
                                            nn.Sigmoid())
    ...
    def forward(self, hidden_states, extra_emb=None, extra_emb_list=None, inference_params=None):
        """
        hidden_states: (B, L, D)
        extra_emb: (B, L, D)
        Returns: same shape as hidden_states
        ...
        if extra_emb_list is not None:
            for i in range(len(extra_emb_list)):
                if i == 0:
                    extra_emb_all = extra_emb_list[i]
                else:
                    extra_emb_all = torch.cat((extra_emb_all,extra_emb_list[i]),dim=1)
        
            extra_emb = torch.mul(self.att(extra_emb_all),hidden_states)
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v0":
                ...
                #                  w=self.input_w,
                #                  h=self.input_h)
                x_c, z_c = xz.flip([-2]).chunk(2, dim=1)
                x_d, z_d = xz.flip([-1,-2]).chunk(2, dim=1)

                extra = extra_emb
                extra_b = extra_emb.flip([-1])
                extra_c = extra_emb.flip([-2])
                extra_d = extra_emb.flip([-1,-2])
                ...
                if not self.if_devide_out:
                    if self.use_norm:
                        out = F.linear(self.norm(rearrange(y + y_b.flip([-1]) + y_c.flip([-2]) + y_d.flip([-1,-2]), "b d l -> b l d")),
                                       self.out_proj.weight, self.out_proj.bias)
                    else:
                        out = F.linear(rearrange(y + y_b.flip([-1]) + y_c.flip([-2]) + y_d.flip([-1,-2]), "b d l -> b l d"),
                                       self.out_proj.weight, self.out_proj.bias)
                else:
                    if self.use_norm:
                        out = F.linear(self.norm(rearrange(y + y_b.flip([-1]) + y_c.flip([-2]) + y_d.flip([-1,-2]), "b d l -> b l d") / 4),
                                       self.out_proj.weight, self.out_proj.bias)
                    else:
                        out = F.linear(rearrange(y + y_b.flip([-1]) + y_c.flip([-2]) + y_d.flip([-1,-2]), "b d l -> b l d") / 4,
                                       self.out_proj.weight, self.out_proj.bias)
